...
import { useIsApprovedERC20 } from "hooks/useIsApproved"
import { useWeb3Context } from "hooks/useWeb3Context"
import React, { useState } from "react"
import { useParams } from "react-router-dom"
import ButtonPrimary from "shared/Button/ButtonPrimary"
import ButtonSecondary from "shared/Button/ButtonSecondary"
import { getMarketplaceAddress, getWBNBAddress } from "utils/addressHelpers"
import { callApprovalForAllERC20 } from "utils/calls"
...
const UserListSection: React.FC<Props> = ({data, reload, setReload}) => {
  const web3Context = useWeb3Context()
  const params = useParams()
  const ethPrice = useETHPrice()
  const [reloadApproval, setReloadApproval] = useState(false)
  const allowance = useIsApprovedERC20(web3Context?.account, getWBNBAddress(), getMarketplaceAddress(), reloadApproval)
  console.log("sniper: data: ", data)

  const [isShowBuy, setIsShowBuy] = useState(false);
  const [isShowOffer, setIsShowOffer] = useState(false);

  const openModalBuy = () => setIsShowBuy(true);
  const closeModalBuy = () => setIsShowBuy(false);
  const openModalOffer = () => setIsShowOffer(true);
  const closeModalOffer = () => setIsShowOffer(false);
  ...
  const handleApprove = async () => {
    await callApprovalForAllERC20(web3Context, getWBNBAddress(), getMarketplaceAddress())
    setReloadApproval(!reloadApproval)
  }

  return (
    <div className="pb-9 pt-14">
      <ModalBuy show={isShowBuy} onCloseModalBuy={closeModalBuy} price={data.listOrAuctionStartPrice} reload={reload} setReload={setReload}/>
      <ModalOffer show={isShowOffer} onCloseModalOffer={closeModalOffer} reload={reload} setReload={setReload}/>
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between">
        <div className="flex-1 flex flex-col sm:flex-row items-baseline p-6 border-2 border-green-500 rounded-xl relative m-2">
          <span className="absolute bottom-full translate-y-1 py-1 px-1.5 bg-white dark:bg-neutral-900 text-sm text-neutral-500 dark:text-neutral-400">
            {data.highestBidPrice > 0? "Highest Bid Price" : "Starting Price"}
          </span>
      ...
      <div className="mt-2 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
        {data.listOrAuctionStartPrice !== 0 && <ButtonPrimary onClick={handleBuy} className="flex-1">
        ...
        <ButtonSecondary onClick={allowance > 2**50? handleOffer : handleApprove} className="flex-1">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
            <path
          ...
          <span className="ml-2.5">{allowance > 2**50? "Make a Offer" : "Enable WETH to Bid"}</span>
        </ButtonSecondary>
      </div>
    </div>
  )
