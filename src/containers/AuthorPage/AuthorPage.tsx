...
import { useGetProfileNFTs } from "hooks/useGetProfileNFTs";
...
const AuthorPage: FC<AuthorPageProps> = ({ className = "" }) => {
  ...
  const {ownedNFTs, mintedNFTs, auctionNFTs, listedNFTs, mintedLoading, ownedLoading} = useGetProfileNFTs(params.account)
            ...
            <div className="pt-5 md:pt-1 md:ml-6 xl:ml-14 flex-grow">
              <div className="max-w-screen-sm ">
                ...
                <div className="flex items-center text-sm font-medium space-x-2.5 mt-2.5 text-green-600 cursor-pointer">
                  <h2 className="inline-flex items-center text-xl sm:text-3xl lg:text-2xl font-semibold">
                    {formatAddress(params.account)}{" "}
                  </h2>
            ...
            <Tab.Panels>
              <Tab.Panel className="">
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-10 mt-8 lg:mt-10">
                  {ownedNFTs&& ownedNFTs.map((item: any, index: number) => (
              ...
              <Tab.Panel className="">
                {/* LOOP ITEMS */}
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-10 mt-8 lg:mt-10">
                  {mintedNFTs && mintedNFTs.map((item: any, index: number) => (
                    <CardNFT data={item} key={index} />
                  ))}
                </div>
              ...
              <Tab.Panel className="">
                {/* LOOP ITEMS */}
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-10 mt-8 lg:mt-10">
                  {auctionNFTs && auctionNFTs.map((item: any, index: number) => (
                    <CardNFT data={item} isLiked key={index} />
                  ))}
                </div>
              ...
              <Tab.Panel className="">
                {/* LOOP ITEMS */}
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-8 gap-y-10 mt-8 lg:mt-10">
                  {listedNFTs && listedNFTs.map((item: any, index: number) => (
                    <CardNFT data={item} key={index} />
                  ))}
                </div>
