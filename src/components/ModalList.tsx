...
export interface ModalListProps {
  show: boolean;
  onCloseModalList: () => void;
  reload: boolean
  setReload: (reload: boolean) => void
}

const ModalList: FC<ModalListProps> = ({ show, onCloseModalList, reload, setReload }) => {
  ...
  const renderContent = () => {
    return (
      <form action="#">
        ...
        <span className="text-sm">Are you sure you want to list your NFT to sell?</span>
        ...
        <div className="mt-4 space-x-3">
          <ButtonPrimary type="button" onClick={handleListItem}>Submit</ButtonPrimary>
          <ButtonSecondary type="button" onClick={onCloseModalList}>
            Cancel
          </ButtonSecondary>
        </div>
      </form>
  ...
  const handleListItem = async () => {
    ...
    if (element) {
      ...
      const result = await callList(web3Context, params.collection, params.id, price)
  ...
  const renderTrigger = async () => {
    ...
    //   // const result = await callList(web3Context?.provider, web3Context?.account, params.collection, params.id, price)
    //   return null
    // }
    // return null
